...
#define SPIKECPLUSPLUS 1
typedef uint32_t SpikeLinkInputState;
enum _SpikeLinkInputState {
    EXCEPTION,
    CLOSED,
    OPENED,
    STARTED,
...
/* SpikeLink USB3.0 I/O device*/

typedef struct SpikeLinkUSB3InitParams {
    int32_t         type;
    char            openByParam[256];
    uint32_t        packetSize;
    uint32_t        queueSize;
} SpikeLinkUSB3InitParams;
...
//    virtual int32_t EnumerateDevice(int8_t *deviceName[],int iDeviceNumMax, int *pDeviceNum) = 0;
    virtual uint16_t SV_CALLTYPE AddRef() = 0;
    virtual uint16_t SV_CALLTYPE Release() = 0;
...
SV_API int32_t EnumDevice(int8_t *deviceName[], int32_t iDeviceNumMax, int32_t *pDeviceNum);
