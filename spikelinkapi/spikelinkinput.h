...
#define SPS10_002_PKT_HEADER_SIZE  16
#define SPS10_002_PKT_LINE_HEADER_SIZE  2
#define SPS10_002_FRAME_LINE_BEGIN  0
#define SPS10_002_FRAME_LINE_END  0XF9
#define SPS10_002_FRAME_WIDTH  50
#define SPS10_002_FRAME_HEIGHT 250
...
class SpikeFramePool {
  public:
    ...
    int32_t Init(int32_t format, int32_t width, int32_t height, int32_t nframe, int32_t freamHeaderSize);
    ...
    static int32_t GetFrameSize2(int32_t format, int32_t width, int32_t height, int32_t freamHeaderSize);
...
class SpikeLinkBaseInput {
  ...
  //  virtual int32_t EnumerateDevice(int8_t *deviceName[],int iDeviceNumMax, int *pDeviceNum);
  ...
  protected:
    ...
    unique_ptr<ISpikeLinkInputObserver> obsver_;
...
class SpikeLinkUSB3 : public SpikeLinkBaseInput {
  public:
    static SpikeLinkUSB3* CreateInstance();
    virtual int32_t Init(SpikeLinkInitParams *initParams, ISpikeLinkInputObserver *obsver);
    virtual int32_t Init(SpikeLinkInitParams *initParams);
    virtual void    Fini();
    virtual int32_t Open();
    ...
    virtual int32_t Start();
    virtual int32_t Stop();
    static int32_t EnumerateDevice(int8_t *deviceName[], int32_t iDeviceNumMax, int32_t *pDeviceNum);
    SpikeLinkUSB3();
    virtual ~SpikeLinkUSB3();
  protected:
  private:
    ...
    int32_t InitLinkDev();
  private:
    unique_ptr<CDriverInterface> devInfo_;
    unique_ptr<SpikeLinkUSB3InitParams> params_;
    unique_ptr<SpikeFramePool> dataPool_;
    unique_ptr<std::thread> readThrd_;
    unique_ptr<std::thread> decodeThrd_;
    int32_t frameSize_;
    // unique_ptr<LoadLibRAII> loadLib_;
    // unique_ptr<CDriverInterface> linkDev_;
        // Endpoint-related
    uint8_t readEP_;
    uint8_t writeEPs_[NUM_EP_PAIRS];
    uint8_t readEPs_[NUM_EP_PAIRS];
    uint8_t nWriteEP_;
    uint8_t nReadEP_;
