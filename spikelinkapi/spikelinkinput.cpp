...
int32_t SpikeFramePool::GetFrameSize2(int32_t format, int32_t width, int32_t height, int32_t freamHeaderSize) {
    int32_t frmSize = 0;
    ...
    return (frmSize + freamHeaderSize);
}
...
int32_t SpikeFramePool::Init(int32_t format, int32_t width, int32_t height, int32_t nframe, int32_t freamHeaderSize) {
    assert(format >= 0);
    assert(width > 0);
    assert(height > 0);
    assert(nframe > 0);
    ...
    int32_t rtv = -1;
    {
        ...
        int32_t frameSize = GetFrameSize2(format, width, height, freamHeaderSize);
...
int32_t SpikeLinkBaseInput::Init(SpikeLinkInitParams *initParams, ISpikeLinkInputObserver *obsver) {
    ...
    if (framePool_->Init(initParams->picture.format, initParams->picture.width,
                         initParams->picture.height, initParams->buff_size, initParams->picture.ppsSize) != 0) {
        return (-1);
    }
...
void SpikeLinkBaseInput::Fini() {
    bExit_ = true;
    ISpikeLinkInputObserver *obsver = nullptr;
    {
        cond_.notify_one();
    }
    Sleep(1);
    if (recvThrd_.get() != nullptr) {
        recvThrd_->join();
        recvThrd_.reset(nullptr);
    }

    if(obsver_ != nullptr) {
        ISpikeLinkInputObserver *obsver = obsver_.release();
        obsver_ = nullptr;
    }
...
bool SpikeLinkBaseInput::IsOpen() {
    return (state_ == _SpikeLinkInputState::OPENED || state_ == _SpikeLinkInputState::STARTED );
}
...
// int32_t SpikeLinkBaseInput::EnumerateDevice(int8_t *deviceName[], int iDeviceNumMax, int *pDeviceNum) {
...
int32_t SpikeLinkDummy::Stop() { 
    ...
    {
        unique_lock<mutex> lock(mtx_);
        state_ = _SpikeLinkInputState::OPENED;
        if (readThrd_ != nullptr) {
            readThrd_->join();
            readThrd_.reset(nullptr);
        }
...
#endif // __GNUC__
                break;
            case svDeviceInterfaceUSB:
                input_.reset(SpikeLinkUSB3::CreateInstance());
                retv = ((SpikeLinkUSB3*)input_.get())->Init(initParams, obsver);
                break;
...
int32_t SpikeLinkInputAdapter::Init(SpikeLinkInitParams *initParams) {
    ...
    int retv = -1;
    {
        ...
        switch (initParams->type) {
            ...
            case svDeviceInterfaceUSB:
                input_.reset(SpikeLinkUSB3::CreateInstance());
                retv = ((SpikeLinkUSB3*)input_.get())->Init(initParams);
                break;
...
void SpikeLinkInputAdapter::Fini() {
    ...
    if (input != nullptr) {
        switch (type_) {
            ...
            case svDeviceInterfaceUSB: {
                unique_ptr<SpikeLinkUSB3> usb3Input((SpikeLinkUSB3*)input);
                //usb3Input->Fini();
                break;
            }   
...
int32_t SpikeLinkInputAdapter::GetState() {
    ...
    int retv = input_->GetState();
    return (retv);
}

int32_t SpikeLinkInputAdapter::EnumerateDevice(int8_t *deviceName[], int32_t iDeviceNumMax, int32_t *pDeviceNum) {
    // assert(input_ != nullptr);
    ...
    //     retv = input_->EnumerateDevice(deviceName, iDeviceNumMax, pDeviceNum);
    // }
    // return (retv); 

    return SpikeLinkUSB3::EnumerateDevice(deviceName, iDeviceNumMax, pDeviceNum);
}
